...
| **证书批量续签** | 提供 **一键批量续签证书** 功能，避免证书过期造成业务中断。 | 批量续签证书、一键批量续签证书功能 |
| **批量建站管理** | 快速批量导入域名，自动完成建站过程。 | **宝塔api批量建站**、批量建站 |
...
* **最新 Release 版本下载：** [点击此处下载最新 `BaoTaApi.exe` 文件] (替换为你的 GitHub Release 链接)
* **文件列表（仅需打包这些）：** `BaoTaApi.exe`，`BaoTaApi.exe.config`，`Newtonsoft.Json.dll` (如果你未选择单文件打包)。
